#!/usr/bin/env node
/**
 * ESLint warning ratchet. Runs ESLint over the source tree and compares the
 * warning count to .eslint-warning-baseline. Fails when the count goes UP,
 * succeeds when it goes down or stays the same. The baseline is committed
 * to git; once a PR lowers it, update the baseline file in the same commit.
 *
 * Usage:
...
try {
  execSync(
    `./node_modules/.bin/eslint . --ext .js,.jsx,.ts,.tsx --format json > "${lintOutputPath}"`,
    { encoding: 'utf8', stdio: ['ignore', 'pipe', 'pipe'], maxBuffer: 64 * 1024 * 1024 },
  );
} catch (err) {
  // ESLint exits non-zero when errors exist. The JSON report still lands in the file.
